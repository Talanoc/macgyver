...
import pandas
import pygame
from constants import (needle, syringe, ether, macgyver, wall, floor,
                       gardien, size, resize_decor)
...
class Maze:
    ...
    def load_maze(self):
        ...
        load maze from xlsx file
            __entrée:maze_name
            __traitement interne à la methode : wall_a/
            (sans self puisque inutilisée ailleurs)
            __sortie:self.wall_display
            (self utilisation en dehors de la methode)
            contient les numeros appartenant
                au fichier xlsx sans les vides
        """
        self.wall_display = []
        wall_a = []
        datas = pandas.read_excel("ressource/labyrinthe.xlsx", self.maze_name)
    ...
    def extract_position_heroes(self):
        ...
        __sortie:self.position_macgyver et self.position_gardien
        (self utilisation en dehors de la methode)
        """
    ...
    def generate_integrate_items(self):
        ...
        __generation de 3 nombres aléatoire ayant pour nom 'floor'
        dans la liste data_maze
